...
int main(void) {
   ...
   int my_rank, comm_sz;
   MPI_Comm comm;
   double start, finish, loc_elapsed, elapsed,loc_serial, serial,loc_ef, ef;
...
#  endif
   ...
   //Tiempo paralelo
   t = clock() - t; 
   serial = ((double)t)/CLOCKS_PER_SEC;
   //Tiempo en serie
   loc_elapsed = finish-start;
   MPI_Reduce(&loc_elapsed, &elapsed, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
   MPI_Reduce(&loc_serial, &serial, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
   MPI_Reduce(&loc_ef, &ef, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
#  ifdef DEBUG
   Print_vector("y", local_y, m, local_m, my_rank, comm);
#  endif
   //Eficencia
   ef = time_taken/(comm_sz*elapsed);

   if (my_rank == 0)
      printf("Tiempo paralelo = %e\n", elapsed);
      printf("Tiempo serial = %e\n", serial);
      printf("Eficiencia = %e\n", ef);
